    ...
    const run = <T>(promise: Promise<T>, options: Options = {}): Promise<T> => {
        ...
        return promise
            .then((val) => {
                ...
                if (withSuccessMsg) {
                    if (isFunction(successMsg)) { Message.success(successMsg(val as T)) } else { Message.success(successMsg || 'Success') }
                }
                return val
            })
            .catch((e) => {
                isLoading.value = false
                if (withErrorMsg) {
                    if (isFunction(errorMsg)) { Message.error(errorMsg(e)) } else { Message.error(errorMsg || e?.message || e?.msg || 'Error') }
                }
                throw e
            })
    }
