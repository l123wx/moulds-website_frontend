<script setup lang="ts">
    ...
    const handleCommand = (command: string) => {
        router.push(switchLocalePath(command as any))
    }
</script>

<template>
    <ClientOnly>
        <el-dropdown :trigger="isMobile ? 'click' : 'hover'" @command="handleCommand">
            <div class="button">
            ...
            <template #dropdown>
                <el-dropdown-menu>
                    <el-dropdown-item v-for="language in languageList" :key="language.id" :command="language.code">
                        {{ language.name }}
                    </el-dropdown-item>
                    <el-dropdown-item v-if="languageList.length === 0" disabled>
        ...
        <template #fallback>
            <el-skeleton style="width: 48px" animated>
                <template #template>
